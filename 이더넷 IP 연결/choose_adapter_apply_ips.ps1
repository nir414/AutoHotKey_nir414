	...
	if ($mode -eq '0') {
		do {
			...
			try {
				...
				Write-Host "[대기중] 응답 수신 중..." -ForegroundColor Yellow
				
				$startTime = Get-Date
				
				# 응답 수신 루프
				while ((Get-Date) - $startTime -lt [TimeSpan]::FromMilliseconds($RECEIVE_TIMEOUT)) {
					try {
						...
						if ($receivedText -match "CN=" -and $receivedText -match "MD=" -and $receivedText -match "VR=") {
							...
							if ($foundDevices.IP -notcontains $deviceInfo.IP) {
								...
								Write-Host "[발견] 새 장비 발견: $($deviceInfo.IP) - $($deviceInfo.Name)" -ForegroundColor Cyan
							}
						}
					}
			...
			if ($foundDevices.Count -eq 0) {
				...
				Write-Host "`n[재실행] 다시 검색하시겠습니까? (y/n)"
				$retryChoice = Read-Host
				if ($retryChoice -eq 'y' -or $retryChoice -eq 'Y') {
					continue  # do-while 루프 재실행
				} else {
					Write-Host "[대기] 아무 키나 누르면 메뉴로 돌아갑니다..."
					pause
					break  # do-while 루프 종료
				}
			} else {
				break  # 장비가 발견되면 루프 종료
			}
		} while ($true)
